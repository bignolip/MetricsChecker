...
/**
 * A Constituent Skeleton only links the types of constituent nodes to determine if there are
 * patterns that can be exploited (i.e. Are there certain construction-skeletons that comprise
 * most NL utterances or is this random?  Would these patterns have to subjected to "transformations"
 * such as ellipses in order to be used/discovered?)
 * 
 * 
 * @author hlil_administrator
 *
 */
public class ConstituentSkeleton
{
	private static int HASHCODE = 0;
	
	private String constituentType;
	
	private ConstituentSkeleton parent;
	
	private List<ConstituentSkeleton> leftToRightChildren;
	...
	private void changeStringRepMessageToParent()
	{
		...
		Stack<Pair<ConstituentSkeleton, Integer>> frontier = 
			new Stack<Pair<ConstituentSkeleton, Integer>>();
		
		frontier.add(new Pair<ConstituentSkeleton, Integer>(this, 0));
		...
		Map<ConstituentSkeleton, Integer> nodeToNumBacktracks =
			new HashMap<ConstituentSkeleton, Integer>();
		...
		// of a node
		Map<ConstituentSkeleton, Integer> nodeToInheritedLeftOffset =
			new HashMap<ConstituentSkeleton, Integer>();
		...
		// parent offset
		Map<ConstituentSkeleton, Integer> nodeToChildOffset =
			new HashMap<ConstituentSkeleton, Integer>();
		...
		while (!frontier.isEmpty())
		{
			...
			if (nodeToInheritedLeftOffset.get(curNode) == null)
			{
				...
				if ((curLeftToRightChildren != null) && !curLeftToRightChildren.isEmpty())
				{
					...
					{
						frontier.push(
							new Pair<ConstituentSkeleton, Integer>(curNode, curStringLineDepth));
						frontier.push(
							new Pair<ConstituentSkeleton, Integer>(curLeftToRightChildren.get(i), newStringLineDepth));
					}
				}
			}
	...
	public ConstituentSkeleton(
			String constituentType)
	{
		this.constituentType = constituentType;

		this.leftToRightChildren = new LinkedList<ConstituentSkeleton>();

		this.hashCode = ConstituentSkeleton.HASHCODE;
		ConstituentSkeleton.HASHCODE++;
	...
	public ConstituentSkeleton getParent()
	{
		return this.parent;
	}

	public List<ConstituentSkeleton> getConstituentChildren()
	{
	...
	{
		if (obj instanceof ConstituentSkeleton)
		{
			ConstituentSkeleton cast = (ConstituentSkeleton)obj;

			if (!this.constituentType.equals(((ConstituentSkeleton) obj).constituentType))
			{
				return false;
			}
			...
			{
				for (int i = 0 ; i < this.numConstituentChildren() ; i++)
				{
